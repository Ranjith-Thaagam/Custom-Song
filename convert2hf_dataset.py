...
def create_dataset(data_dir="./data", repeat_count=2000, output_name="zh_lora_dataset"):
    ...
    for ext in audio_extensions:
        for song_path in data_path.glob(ext):
            # Get the base name without extension
            base_name = song_path.stem
            prompt_path = data_path / f"{base_name}_prompt.txt"
            lyric_path = data_path / f"{base_name}_lyric.txt"  # Fixed: should be _lyric.txt not _lyrics.txt
            try:
